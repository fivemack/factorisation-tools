...
# rewrite project names to come out right in gnuplot
	...
	elsif ($pn =~ m/GW_([0-9]*)_([0-9]*)/ or
	       $pn =~ m/^([0-9]+)_([0-9]+)m$/) { $pn = "$2*$1^$2-1"; }
	elsif ($pn =~ m/GC_([0-9]*)_([0-9]*)/ or 
	       $pn =~ m/^([0-9]+)_([0-9]+)p$/) { $pn = "$2*$1^$2+1"; }
	elsif ($pn =~ m/C([0-9]*)_([0-9]*)_([0-9]*)/) {
	    # this could be an aliquot or an HCN
	    if ($2*log($3)+$3*log($2) < 1000)
	    { $pn = "C$1 from $2^$3+$3^$2"; }
	    else
	    { $pn = "C$1 from aliquot sequence $2 step $3"; }
	}
	elsif ($pn =~ m/^([0-9]+)([mp])([0-9]+)_([0-9]+)$/)
	{
	    if ($2 eq "p") { $pn = "$1^$4+$3^$4"; }
	    else           { $pn = "$1^$4-$3^$4"; }
	}
	...
	elsif ($pn =~ m/(^[0-9]{5})_([0-9]*)$/)
	{
	    ...
	    {
		# this notation is used inconsistently :(
		if ($2 < 150)
		{
